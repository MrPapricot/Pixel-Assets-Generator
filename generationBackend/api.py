...
def process_image_task(task_id: str, image_path: str, config: Dict[str, Any]):
    ...
    try:
        ...
        # Создаем конфигурацию для трансформера
        config_json = json.dumps(config)
        
        # Инициализируем трансформер
        tasks_store[task_id].progress = 20
        transformer = PixelArtTransformer(io.StringIO(config_json))
...
@app.post("/api/v1/transform/json", response_model=TransformResponse)
async def transform_with_config(
    background_tasks: BackgroundTasks,
    config: TransformConfig,
    file: UploadFile = File(..., description="Изображение для обработки")
):
    """
    Обработка изображения с полной конфигурацией в JSON
    
    - **config**: Полная конфигурация обработки
...
@app.post("/api/v1/transform/base64", response_model=TransformResponse)
async def transform_from_base64(
    background_tasks: BackgroundTasks,
    ...
    - **image_base64**: Изображение в формате base64 (можно с префиксом data:image/...)
    - **config**: Опциональная JSON конфигурация
    """
    ...
    upload_path = os.path.join(UPLOAD_DIR, f"{task_id}.png")
...
@app.get("/api/v1/result/{task_id}/base64")
async def get_result_base64(task_id: str):
    """
    ...
    base64_str = image_to_base64(result_path)
...
async def get_config_schema():
    """
    Получение JSON схемы конфигурации
    """
    return TransformConfig.schema()
