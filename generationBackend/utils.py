...
class PixelArtTransformer:
    ...
    def process_image(self, input_path, output_path):
        ...
        img = self.apply_stylization(img, stylization, func['asset_type']) #!!!!!!!!!!!!!!!!!!!!!!!
    ...
    def apply_stylization(self, img, stylization, asset_type):
        """
        ...
        asset_contexts = {
            "character": {
                "minimalist": "Keep character silhouette recognizable, minimal facial features.",
                "clean": "Smooth character outlines, readable animations frames style.",
                "dithered": "Dithering on character shadows and texture details.",
                "high_detail": "Detailed character features, expressive pixel art portrait quality."
            },
            "prop": {
                "minimalist": "Object reduced to iconic shape, single color areas.",
                "clean": "Clear object definition, functional design readability.",
                "dithered": "Dithering on metallic or textured surfaces.",
                "high_detail": "Intricate prop details, material texture indication."
            },
            "tile": {
                "minimalist": "Tileable minimal pattern, basic terrain indication.",
                "clean": "Clean tileable texture, seamless edges.",
                "dithered": "Dithering for texture variation in tiles.",
                "high_detail": "Detailed tile texture with edge transitions."
            }
        }

        base_prompt = stylization_prompts.get(stylization, stylization_prompts["clean"])
        asset_hint = asset_contexts.get(asset_type, {}).get(stylization, "")

        full_prompt = f"{base_prompt} Asset type: {asset_type}. {asset_hint}"
        ...
        # Вызываем ComfyUI
        try:
            result = img#self.call_comfyui_api(img, full_prompt, f"stylization_{stylization}")
    ...
    def call_comfyui_api(self, img: Image.Image, prompt: str, projection_type: str) -> Image.Image:
        ...
        Args:
            ...
            prompt: текстовая инструкция для Qwen
            projection_type: тип проекции (для логирования)
        ...
        # Нода 78: LoadImage
        if "78" in workflow:
            workflow["78"]["inputs"]["image"] = f"data:image/png;base64,{img_base64}"
            print(f"✅ Изображение загружено в ноду 78")

        # Нода 435: Prompt
        if "435" in workflow:
            workflow["435"]["inputs"]["value"] = prompt
            print(f"✅ Промпт установлен: '{prompt}'")

        # Нода 433:443: Включение Lightning LoRA (опционально)
        # true = использовать Lightning (4 шага, быстрее)
        # false = обычный режим (20 шагов, качественнее)
        ...
        # Опционально: можно менять seed для разнообразия результатов
    ...
    def _wait_for_comfyui_result(self, comfyui_url: str, prompt_id: str, timeout: int = 180) -> Image.Image:
        """
    ...
    def apply_projection(self, img, projection, asset_type):
        ...
        prompts = {
            "side_view": (
                ...
                "Clean pixel art style, sharp edges, flat colors."
            ),
            "top_down": (
                "Transform the object to a top-down view (bird's eye view). "
                "Looking directly from above. "
                "Pixel art style, clean outlines, game asset."
            ),
            "isometric_2_1": (
                "Transform the object to an isometric dimetric projection with 2:1 ratio. "
                "View from 30 degree angle, pixel art isometric style. "
                "Clean lines, game asset sprite."
            ),
            "isometric_1_1": (
                "Transform the object to a true isometric projection (1:1 ratio). "
                "Perfect 30/60 degree angles, classic isometric pixel art. "
                "Game asset, clean style."
            )
        }
        ...
        # Добавляем контекст типа ассета
        asset_context = {
            "character": "character sprite, animated style",
            "prop": "prop object, item, equipment",
        ...
        # Вызываем ComfyUI
        try:
            result = img#self.call_comfyui_api(img, full_prompt, projection)
